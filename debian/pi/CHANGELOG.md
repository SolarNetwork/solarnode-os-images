...
no longer based on RaspiOS. Rather they are derived from "vanilla" Debian Raspberry Pi images. They
are also hardware-specific, so for example `pi3` is for the Pi 3B+ while `pi4` is for the Pi 4.
